...
test:
	...
	R --slave -e 'Sys.setenv(NOT_CRAN="true"); library(testthat); setwd(file.path(.libPaths()[1], "h5testR", "tests")); system.time(test_check("h5testR", filter="${file}", reporter=ifelse(nchar("${r}"), "${r}", "summary")))'
...
check: build
	-R CMD check --as-cran h5testR_$(VERSION).tar.gz
	rm -rf h5testR.Rcheck/
